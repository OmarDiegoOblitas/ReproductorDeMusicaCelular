...
let musicIndex = Math.floor((Math.random() * allMusic.length) + 1);
    ...
    window.addEventListener("load", ()=>{
        loadMusic(musicIndex); // calling load music function aonce window
        playingNow();
    })
...
//update pregress bar width according to music current time 
mainAudio.addEventListener("timeupdate", (e)=>{
    const currentTime = e.target.currentTime;//getting current time of song 
    const duration = e.target.duration; //getting total durtation of song 
...
repeatBtn.addEventListener("click", ()=>{
    //first we get the innerText of the icon then we'll change acordingly
    ...
    switch(getText) {
        case "repeat": //if this icon is repeat then change it to repeat_one
            ...
            break; 
        case "repeat_one": //if icon icon is shuffle then change it to repeat
    ...
    //let's do different changes on different icon click using switch  
      switch (getText) {
        case "repeat": //if this icon is repeat then simply we call the nextMusic function so the next song will play
            nextMusic();
            break; 
        case "repeat_one": //if icon icon is repeat_one then we'll change the current playing song current time to 0 so osng will play from beginning
        ...
        //generating random index between the max range of array length
            ...
            } while (musicIndex == randIndex); //this loop until the next random number won't be the same of current music index
...
for (let i = 0; i < allMusic.length; i++){
    ...
    liAudioTag.addEventListener("loadeddata", ()=>{
        ...
        //adding t duration atrribute which we'll use below
...
//let's work on play particular song on click
    ...
    function playingNow(){
        for (let j = 0; j < allLiTags.length; j++) {
            ...
            //if there is an li tag which li-indez is equal to musicIndex
            if(allLiTags[j].classList.contains("playing")){
                allLiTags[j].classList.remove("playing");
                //let's get that audio duration value and pass to .audio-duration inneretext
            ...
            //adding onclick attibute in all li tagss 
